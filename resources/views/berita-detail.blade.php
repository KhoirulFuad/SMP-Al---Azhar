...
@section('title', $berita->title . ' - SMP Islam Al-Azhar 17')
...
<style>
  ...
  .hero-wrapper {
    position: relative;
    width: 100%;
    /* UBAH: Tinggi dinaikkan agar gambar area terlihat lebih besar */
    height: 500px; 
    background-color: #000;
    overflow: hidden;
    /* UBAH: Lengkungan sudut dihapus (edgenya buang) */
    border-radius: 0; 
    display: flex;
    align-items: center;
  ...
  .hero-blur-bg {
    ...
    /* Scale sedikit lebih besar untuk menghilangkan garis putih di pinggiran akibat blur */
    transform: scale(1.1);
    z-index: 1;
  }
  ...
  .detail-hero-main {
    position: relative;
    z-index: 2;
    /* Gambar utama akan menyesuaikan tinggi container baru */
    max-width: 100%;
    max-height: 100%;
    object-fit: contain;
    display: block;
  ...
  .detail-hero-placeholder { 
    ...
    color: #9ca3af; 
    font-size: 14px; 
    /* UBAH: Lengkungan sudut dihapus di placeholder juga */
    border-radius: 0; 
  }
  ...
  .divider { border: none; border-top: 1.5px solid #e5e5e5; margin: 0 0 28px; }
  ...
  /* Responsif: Sedikit penyesuaian tinggi di layar HP agar tidak terlalu dominan */
...
@section('content')
  ...
  @if ($berita->image)
    <div class="hero-wrapper">
      <img src="{{ asset('storage/' . $berita->image) }}" alt="background-blur" class="hero-blur-bg">
      
      <img src="{{ asset('storage/' . $berita->image) }}" alt="{{ $berita->title }}" class="detail-hero-main">
    </div>
  @else
    {{-- Placeholder juga disesuaikan tanpa edge --}}
  ...
  <div class="detail-wrap">
    ...
    <div><span class="detail-category">Berita</span></div>
    ...
    <div class="detail-meta">
      <span>
        ...
        {{ \Carbon\Carbon::parse($berita->news_date)->translatedFormat('d F Y') }}
      </span>
    </div>
    ...
    <div style="margin-top: 56px;">
      ...
      <div class="lainnya-grid">
        ...
        <a href="{{ route('berita.show', $other->slug) }}" class="lainnya-card">
          @if ($other->image)
          ...
          <div class="lainnya-card-body">
            ...
            <p class="lainnya-card-date">{{ \Carbon\Carbon::parse($other->news_date)->translatedFormat('d F Y') }}</p>
          </div>
        </a>
        @endforeach
      </div>
