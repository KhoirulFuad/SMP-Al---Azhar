...
@section('title', 'Agenda Sekolah - SMP Islam Al-Azhar 17')
...
<div class="hero-banner">
  <img src="{{ asset('storage/beranda/gedungsmp2.png') }}" alt="Agenda Sekolah">
  <div class="hero-overlay">
    <h1 style="color:#fff; font-size:36px; font-weight:700; letter-spacing:6px; margin:0;">AGENDA SEKOLAH</h1>
    <p style="color:rgba(255,255,255,0.7); font-size:14px; letter-spacing:0.06em; margin:0;">Jadwal kegiatan & momen penting sepanjang tahun ajaran</p>
  </div>
</div>
...
<section class="relative overflow-hidden" style="background-color:#F9F6F0; min-height:60vh;">
  ...
  <div class="page-wrapper">
    ...
    <div class="main-content">
      ...
      <div class="agenda-card" data-aos="fade-up" data-aos-delay="{{ $loop->index * 80 }}">

        {{-- 1. NOMOR RANKING --}}
        <div class="agenda-rank">
        ...
        <div class="agenda-img">
          ...
          @else
          <div class="agenda-img-placeholder">No Image</div>
          @endif
        </div>

        {{-- 3. KONTEN TEKS --}}
        <div class="agenda-content">
          <a href="{{ route('agenda.show', $item->id) }}" class="agenda-title">{{ $item->title }}</a>
          <p class="agenda-desc">{{ Str::limit(strip_tags($item->content), 150) }}</p>
          <p class="agenda-date"> {{ \Carbon\Carbon::parse($item->agenda_date)->translatedFormat('d F Y') }}</p>
          <a href="{{ route('agenda.show', $item->id) }}" class="read-more">Lihat Selengkapnya →</a>
        </div>

      </div>
      @empty
      <p style="text-align:center; color:#6b7280; padding:60px 0;">Belum ada data agenda sekolah.</p>
      @endforelse
      ...
      <div class="pagination-wrap" data-aos="fade-up">
        ...
        @else
        <a href="{{ $agendas->previousPageUrl() }}" class="page-btn">Sebelumnya</a>
        @endif

        @foreach ($agendas->links()->elements as $element)
        ...
        <a href="{{ $agendas->nextPageUrl() }}" class="page-btn">Selanjutnya</a>
        @else
